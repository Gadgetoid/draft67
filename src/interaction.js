...
export function attachInteraction({ dom, world, rig, ui, onChange }) {
  ...
  function edit(remove) {
    if (rig.blueprint) return; // no editing in blueprint preview
    const hit = world.pick(ray);
    if (!hit) return;
    if (remove) {
      if (hit.remove) { world.remove(...hit.remove); onChange?.(); }
    } else if (!world.has(...hit.place)) {
  ...
  dom.addEventListener('pointerdown', (e) => { downX = e.clientX; downY = e.clientY; downBtn = e.button; });
  ...
  // First-person mode: pointer-locked, edit from crosshair.
